...
export const ShareBox = ({ car }) => {
  const showBox = (state) => state.car.showShare;
...
function Form({ car }) {
  ...
  const shareCar = async (event) => {
    ...
    await axios
      .post(
        "http://localhost:8000/car/share",
        {
          car: car.car.vin,
          share_user: event.target.email.value,
        },
        {
          headers: {
  ...
  return (
    <form onSubmit={shareCar}>
      <div>
        <div className="box-header">
          <div className="text-title">Share Car</div>
          <svg onClick={() => dispatch({ type: "car/hideShare" })}>
            <use href="#close" />
          </svg>
        </div>
      </div>
      <div className="content">
        <div className="text-subhead">Email</div>
        <input
          type="email"
          id="email"
          name="email"
      ...
      <div className="login-button">
        <button className="text-button">Share Car</button>
      </div>
    </form>
  );
}
